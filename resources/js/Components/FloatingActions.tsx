...
export function FloatingActions() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { from: "bot", text: "Hi! I'm the Zamsam assistant. How can we help you with your refrigeration or AC today?" },
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      // Format history payload for Gemini backend (excluding the initial greeting)
      ...
      if (data.success && data.reply) {
        ...
        setMessages((prev) => [
          ...
          {
            from: "bot",
            text: data.message || "Sorry, I am having trouble responding right now. Please reach us on WhatsApp!",
          },
        ]);
      }
    } catch {
      setMessages((prev) => [
        ...
        {
          from: "bot",
          text: "Connection error. Please check your internet or reach us directly on WhatsApp.",
        },
      ]);
    } finally {
      setLoading(false);
    ...
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3">
      {open && (
        <div className="w-[320px] overflow-hidden rounded-md border-b-[3px] border-brand-red bg-white shadow-2xl ring-1 ring-black/10 sm:w-[360px]">
          ...
          {/* Messages Feed */}
          <div className="flex h-72 flex-col gap-2 overflow-y-auto bg-ice px-4 py-4 text-sm">
            {messages.map((m, i) => (
              <div
                key={i}
                className={
                  m.from === "bot"
                    ? "mr-auto max-w-[85%] rounded-md bg-white px-3 py-2 text-navy-950 ring-1 ring-black/5"
                    : "ml-auto max-w-[85%] rounded-md bg-navy-900 px-3 py-2 text-white"
                }
              >
                {m.text}
              </div>
            ...
            {loading && (
              <div className="mr-auto max-w-[80%] rounded-md bg-white/60 px-3 py-2 text-xs italic text-navy-950/60">
                Typing...
              </div>
            )}
          </div>
          ...
          <form onSubmit={handleSend} className="flex items-center gap-2 border-t border-black/5 bg-white p-3">
            <input
              ...
              maxLength={500}
              disabled={loading}
              className="flex-1 rounded-sm bg-ice px-3 py-2 text-sm outline-none placeholder:text-navy-950/40 focus:ring-2 focus:ring-brand-red disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={loading}
